    ...
    public class AuthorizationService : IAuthorizationService, IService
    {
        ...
        public async Task<ClaimsPrincipal> ValidateToken(string token)
        {
            ...
            try
            {
                ...
                if (!tokenValidationConfig.UseRotationalKeys)
                {
                    if(!string.IsNullOrWhiteSpace(tokenValidationConfig.IssuerSigningKey))
                    {
                        tokenValidationParmaeters.IssuerSigningKey = new SymmetricSecurityKey(Encoding.ASCII.GetBytes(tokenValidationConfig.IssuerSigningKey));
                    }

                }
                else
                {                     
                    tokenValidationParmaeters.IssuerSigningKeys = signingKeys;
                }
